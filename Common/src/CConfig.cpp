    ...
    switch (Kind_Solver) {
      ...
      case NEMO_EULER:
        ...
        if(Kind_FluidModel == USER_DEFINED_NONEQ){ 
          if ((GasModel != "N2") && (GasModel != "AIR-5") && (GasModel != "AR"))
          SU2_MPI::Error("The GAS_MODEL given as input is not valid. Choose one of the options: N2, AIR-5, AR", CURRENT_FUNCTION);
        }
        break;
