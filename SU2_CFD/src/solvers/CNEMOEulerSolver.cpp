  ...
  for (iPoint = 0; iPoint < nPointDomain; iPoint++) {
    ...
    if (!adjoint) {
      for (iVar = 0; iVar < nVar; iVar++) {
        ...
        if(Res != Res) cout<<"ERROR RES in Point:" <<iPoint <<" And Var:"<<iVar<<endl;
...
void CNEMOEulerSolver::BC_Euler_Wall_Dev(CGeometry *geometry, CSolver **solver_container, CNumerics *conv_numerics,
                                     CNumerics *visc_numerics, CConfig *config, unsigned short val_marker) {
...
//      }
      ...
      for (iDim = 0; iDim < nDim; iDim++)
        V_reflected[iDim+VEL_INDEX] = nodes->GetVelocity(iPoint,iDim) - 2.0 * ProjVelocity_i*UnitNormal[iDim];
      ...
      /*--- Pass supplementary information to Numerics class---*/
      conv_numerics->SetdPdU(nodes->GetdPdU(iPoint), nodes->GetdPdU(iPoint));
      conv_numerics->SetdTdU(nodes->GetdTdU(iPoint), nodes->GetdTdU(iPoint));
      conv_numerics->SetdTvedU(nodes->GetdTvedU(iPoint), nodes->GetdTvedU(iPoint));
      conv_numerics->SetEve(nodes->GetEve(iPoint), nodes->GetEve(iPoint));
      ...
      if (viscous) {
        ...
        for (iVar = 0; iVar < nDim; iVar++) // loops over the velocity component gradients
          for (iDim = 0; iDim < nDim; iDim++) // loops over the entries of the above
            Grad_Reflected[iVar+VEL_INDEX][iDim] = GradNormVel[iDim]*UnitNormal[iVar] + GradTangVel[iDim]*Tangential[iVar];
...
void CNEMOEulerSolver::BC_Sym_Plane_Dev(CGeometry *geometry, CSolver **solver_container,
                                    CNumerics *conv_numerics, CNumerics *visc_numerics, CConfig *config, unsigned short val_marker) {

  /*--- Call the Euler wall routine ---*/
  BC_Euler_Wall_Dev(geometry, solver_container, conv_numerics, visc_numerics, config, val_marker);
